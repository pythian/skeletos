...
// module augmentation for joi

declare module "joi-full" {
...
// --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
        ...
        alternatives(types: Joi.Schema[]): Joi.AlternativesSchema;
        alternatives(type1: Joi.Schema, type2: Joi.Schema, ...types: Joi.Schema[]): Joi.AlternativesSchema;
         ...
         */
        lazy(cb: () => Joi.Schema): Joi.Schema;

        /**
         * Validates a value using the given schema and options.
         */
        validate<T>(value: T): Joi.ValidationResult<T>;
        validate<T, R>(value: T, callback: (err: Joi.ValidationError, value: T) => R): R;

        validate<T>(value: T, schema: Joi.Schema): Joi.ValidationResult<T>;
        validate<T>(value: T, schema: Object): Joi.ValidationResult<T>;
        validate<T, R>(value: T, schema: Joi.Schema, callback: (err: Joi.ValidationError, value: T) => R): R;
        validate<T, R>(value: T, schema: Object, callback: (err: Joi.ValidationError, value: T) => R): R;

        validate<T>(value: T, schema: Joi.Schema, options: Joi.ValidationOptions): Joi.ValidationResult<T>;
        validate<T>(value: T, schema: Object, options: Joi.ValidationOptions): Joi.ValidationResult<T>;
        validate<T, R>(value: T, schema: Joi.Schema, options: Joi.ValidationOptions, callback: (err: Joi.ValidationError, value: T) => R): R;
        validate<T, R>(value: T, schema: Object, options: Joi.ValidationOptions, callback: (err: Joi.ValidationError, value: T) => R): R;
        /**
         ...
         * @param message - optional message string prefix added in front of the error message. may also be an Error object.
         */
        attempt<T>(value: T, schema: Joi.Schema, message?: string | Error): T;
         ...
         */
        reach(schema: Joi.Schema, path: string): Joi.Schema;


        /**
         * Creates a new Joi instance customized with the extension(s) you provide included.
         */
        extend(extention: Joi.Extension): any;

        /**
         * Whitelists a value
         */
        allow(value: any, ...values: any[]): Joi.Schema;
        allow(values: any[]): Joi.Schema;

        /**
         * Adds the provided values into the allowed whitelist and marks them as the only valid values allowed.
         */
        valid(value: any, ...values: any[]): Joi.Schema;
        valid(values: any[]): Joi.Schema;
        only(value: any, ...values : any[]): Joi.Schema;
        only(values: any[]): Joi.Schema;
        equal(value: any, ...values : any[]): Joi.Schema;
        equal(values: any[]): Joi.Schema;

        /**
         * Blacklists a value
         */
        invalid(value: any, ...values: any[]): Joi.Schema;
        invalid(values: any[]): Joi.Schema;
        disallow(value: any, ...values : any[]): Joi.Schema;
        disallow(values: any[]): Joi.Schema;
        not(value: any, ...values : any[]): Joi.Schema;
        not(values: any[]): Joi.Schema;
         ...
         * Annotates the key
         */
        description(desc: string): Joi.Schema;

        /**
         * Annotates the key
         */
        notes(notes: string): Joi.Schema;
        notes(notes: string[]): Joi.Schema;

        /**
         * Annotates the key
         */
        tags(notes: string): Joi.Schema;
        tags(notes: string[]): Joi.Schema;

        /**
         * Attaches metadata to the key.
         */
        meta(meta: Object): Joi.Schema;

        /**
         * Annotates the key with an example value, must be valid.
         */
        example(value: any): Joi.Schema;

        /**
         * Annotates the key with an unit name.
         */
        unit(name: string): Joi.Schema;

        /**
         * Overrides the global validate() options for the current key and any sub-key.
         */
        options(options: Joi.ValidationOptions): Joi.Schema;

        /**
         * Sets the options.convert options to false which prevent type casting for the current key and any child keys.
         */
        strict(isStrict?: boolean): Joi.Schema;
         ...
         * Converts the type into an alternatives type where the conditions are merged into the type definition where:
         */
        when<U>(ref: string, options: Joi.WhenOptions<U>): Joi.AlternativesSchema;
        when<U>(ref: Joi.Reference, options: Joi.WhenOptions<U>): Joi.AlternativesSchema;

        /**
         * Overrides the key name in error messages.
         */
        label(name: string): Joi.Schema;

        /**
         * Outputs the original untouched value instead of the casted value.
         */
        raw(isRaw?: boolean): Joi.Schema;

        /**
         * Considers anything that matches the schema to be empty (undefined).
         * @param schema - any object or joi schema to match. An undefined schema unsets that rule.
         */
        empty(schema?: any) : Joi.Schema;
